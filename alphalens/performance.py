...
def factor_returns(factor_data,
    ...
    # === 修复 weights.index 重复 level name ===
    if isinstance(weights.index, pd.MultiIndex) and weights.index.names.count("date") > 1:
        weights.index = pd.MultiIndex.from_tuples(
            [(d1, asset) for (d1, d2, asset) in weights.index],
            names=["date", "asset"]
        )

    # === 修复 factor_data.index 重复 level name ===
    if isinstance(factor_data.index, pd.MultiIndex) and factor_data.index.names.count("date") > 1:
        factor_data.index = pd.MultiIndex.from_tuples(
            [(d1, asset) for (d1, d2, asset) in factor_data.index],
            names=["date", "asset"]
        )
